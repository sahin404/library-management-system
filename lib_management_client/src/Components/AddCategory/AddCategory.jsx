...
const AddCategory = ({ setComplete }) => {
    ...
    const handleategory = async e => {
        ...
        const shortDesc = e.target.desc.value;
        const cat = { bookName, shortDesc, image: showName }

        if (bookName && showName && shortDesc) {
            const res = await axios.post('http://localhost:5000/add-category', cat, {
                headers: {
            ...
            <section className="relative flex flex-wrap lg:items-center">
                <div className="w-full px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
                    <div className="mx-auto max-w-lg text-center">
                        <h1 className="text-2xl font-bold sm:text-xl">Get started today with Library Management System</h1>
                    </div>

                    <form onSubmit={handleategory} className="mx-auto mb-0 mt-8 max-w-md space-y-8">
                        <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                required
                                className="w-full rounded-lg border border-black p-4 pe-12 text-sm shadow-sm"
                                placeholder="Enter short description about categories"
                            />
                        </div>
